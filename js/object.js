...
Background.prototype = {
	draw:function(){
		...
		context.drawImage(this.img,0,0,this.w,this.h,0,
			this.y,canvas.width,canvas.height);
		context.drawImage(this.img,0,0,this.w,this.h,0,
			this.y-canvas.height,canvas.width,canvas.height);
		this.y+=this.speed;
	}
}
...
Enemy.prototype = {
	draw:function(bullet){
		...
		for(var i = 0;i<this.objArr.length;i++){
			...
			context.drawImage(this.img[this.objArr[i].imgIndex],
				this.objArr[i].x,this.objArr[i].y)
		}		
	}
}
//---------------敌机--------------